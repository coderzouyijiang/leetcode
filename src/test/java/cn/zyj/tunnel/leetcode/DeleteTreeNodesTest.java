...
import static cn.zyj.tunnel.utils.InputUtil.parseIntArray;
    ...
    public int deleteTreeNodes2(int nodes, int[] parent, int[] value) {
        ...
        for (int i = nodes - 1, j = i; i >= 0; i--) {
            if (parent[i] == parent[j]) {
                ...
                int pid = parent[j];
                sum += value[pid];
//                System.out.printf("(%s,%s]=%s,parent[%s]=%s\n", i, j, sum, j, pid);
                if (sum == 0) {
                    for (int k = j; k > i; k--) {
                        value[k] = 0;
                    }
...
//                System.out.println(Arrays.toString(value));
    ...
    public int deleteTreeNodes3(int nodes, int[] parent, int[] value) {
...
//        输出：2
    ...
    public void test2() {
        ...
        for (int id = nodes - 1; id >= 0; id--) {
            if (id < parent[id]) {
                System.out.println("id=" + id + ",parent[id]=" + parent[id]);
            }
        }
        ...
        // 359
        Assert.assertEquals(326, deleteTreeNodes2(nodes, parent, value));

    }

    @Test
    public void test22() throws Exception {
        BufferedReader reader = new BufferedReader(new FileReader("/Users/zouyijiang/workspace/tunnel/src/test/java/cn/zyj/tunnel/data/deleteTreeNode_8000.txt"));
        int nodes = Integer.valueOf(reader.readLine());
        int[] parent = parseIntArray(reader.readLine());
        int[] value = parseIntArray(reader.readLine());
        Assert.assertEquals(7509, deleteTreeNodes3(nodes, parent, value)); // 7508
        log.info("");
    }
    ...
    public void test3() {
        int nodes = 11;
        int[] parent = {-1, 0, 0, 1, 1, 2, 2, 5, 5, 6, 6};
        int[] value = {0, 1, 6, 0, 4, -1, -1, -1, -1, -1, -1};
        Assert.assertEquals(3, deleteTreeNodes3(nodes, parent, value));
    }
